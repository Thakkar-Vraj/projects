...
// output to ST7735 0.000cm to 2.000cm
 // write this
  uint32_t intPart = n / 1000;     // Integer part (0, 1, or 2)
  uint32_t decPart = n % 1000;     // Decimal part (0 to 999)
  
  // Format the string with the correct number of leading zeros
  char string[10];
  if(decPart < 10){
    // For values like 0.001 to 0.009 or 1.001 to 1.009
    printf(string, "%d.00%d", intPart, decPart);
  }
  else if(decPart < 100){
    // For values like 0.010 to 0.099 or 1.010 to 1.099
    printf(string, "%d.0%d", intPart, decPart);
  }
  else{
    // For values like 0.100 to 0.999 or 1.100 to 1.999
    printf(string, "%d.%d", intPart, decPart);
  }
